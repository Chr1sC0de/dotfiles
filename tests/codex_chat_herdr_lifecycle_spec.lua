...
tests["attachment loss preserves a live Herdr agent"] = function()
	...
	assert_equal(close_calls, 0, "tab not closed")

	herdr.agent_exists = original_agent_exists
	herdr.close_tab = original_close_tab
	vim.api.nvim_buf_delete(session.bufnr, { force = true })
end

tests["agent exit closes the owned tab and deletes the buffer"] = function()
	...
	herdr.close_tab = function(target)
		assert_equal(target.herdr_tab_id, "w1:t2", "closed tab")
		close_calls = close_calls + 1
	end
	...
	assert_equal(close_calls, 1, "fallback tab close")
